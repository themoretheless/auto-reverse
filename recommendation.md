# 860 рекомендаций, проблем и улучшений (500 базовых + N01-N360 follow-up)
...
N140. [Improve] Свернуть 860 рекомендаций в executable roadmap на 20-30 задач.
...
N269. [Problem] `src/ui.rs` разросся до большого God-file: settings, theme helpers, debug console, lifecycle и export в одном месте.
N270. [Improve] Разбить `src/ui.rs` на `ui/app.rs`, `ui/settings.rs`, `ui/devices.rs`, `ui/permissions.rs`, `ui/debug_console.rs`, `ui/theme.rs`.
...
N279. [Problem] `hid::recent_wheel_device` и `recent_wheel_device_name` читают один Mutex двумя отдельными вызовами.
N280. [Improve] Вернуть единый `recent_wheel_snapshot(max_age) -> Option<WheelSnapshot>`.
...
N303. [Problem] `tap_thread_running` is still an inferred bool from a 200ms timeout.
N304. [Improve] Replace `TapStartOutcome` timeout heuristic with explicit runtime channel: started, already-running, failed, stopped.
N305. [Problem] Immediate `Ok(())` from `install_and_run` still conflates lock contention and clean stopped run loop.
N306. [Improve] Make `install_and_run` return `TapRunOutcome::AlreadyRunning` instead of `Ok(())`.
...
N309. [Problem] Debug Console can grow visually empty if no events yet, with no first empty-state copy.
N310. [Improve] Add polished empty state: "Scroll to see live decisions" without tutorial clutter.
...
N318. [Improve] Add `QA.md` with design-handoff checklist and date/person columns.
...
N321. [Problem] No app icon `.icns`; Finder/System Settings still lack branded app identity.
N322. [Improve] Generate `.icns` from the same opposing-arrows concept, separate from menu-bar glyph.
N323. [Problem] Bundle Info.plist still has no `CFBundleIconFile`.
N324. [Improve] Add icon asset pipeline in `scripts/build-app-bundle.sh`.
...
N327. [Problem] The settings window still has no first-run permission onboarding flow.
N328. [Improve] Add compact permission-first state when both permissions are missing.
...
N335. [Problem] The design handoff chose native quiet UI, but code-level theme tokens are still helpers inside `src/ui.rs`.
N336. [Improve] Move colors/spacings/radii into `ui/theme.rs`, with names matching the handoff palette.
...
N338. [Improve] Add a short `DESIGN.md` summary and optionally commit sanitized design references/screenshots if allowed.
N339. [Problem] `recommendation.md` is now too large to serve as daily roadmap.
N340. [Improve] Create `ROADMAP.md` with top 25 executable tasks distilled from N-items.
...
N343. [Problem] No CI, so local green checks can still be skipped by future pushes.
N344. [Improve] Add GitHub Actions for fmt, check, no-default-features, clippy all-targets, test, bundle smoke on macOS.
N345. [Problem] `cargo check --no-default-features` can miss GUI-only AppKit breakage.
N346. [Improve] Keep both GUI default check and no-default check in CI.
...
N360. [Done] The large list is now navigable from `architecture.md` and `readme.md` through collapsed Top 500 mirrors; a smaller executable `ROADMAP.md` is still a future improvement (see N340).
