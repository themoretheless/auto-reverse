# 660 рекомендаций, проблем и улучшений (500 базовых + N01-N160 follow-up)

Список обновлен после merge ветки `worktree-rust-impl`, повторного SOLID/DRY follow-up, локального app-bundle slice, исправления menu bar bundle identity и повторного review 2026-07-04. Он отражает текущий код: macOS event tap, TOML config, отдельный CLI parser, permission checks, raw-input guard, step size, LaunchAgent start at login для CLI, `SMAppService` login item для GUI, JSON startup diagnostics, GUI `.app` bundle, menu bar item и открытые gaps до Scroll Reverser parity.
...
N99. [Problem] Двойной запуск `.app` может создать вторую иконку, даже если tap защищен lock.
N100. [Improve] Добавить GUI single-instance lock или IPC focus command.
...
N123. [Problem] При save error shared_config не обновляется, поэтому UI и runtime могут разойтись.
N124. [Improve] Решить invariant: either "runtime follows UI even if disk failed" or "runtime follows last saved config"; показать это в UI.
...
N140. [Improve] Свернуть 660 рекомендаций в executable roadmap на 20-30 задач.
