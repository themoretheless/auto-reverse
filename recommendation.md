# 760 рекомендаций, проблем и улучшений (500 базовых + N01-N260 follow-up)

Список обновлен после merge ветки `worktree-rust-impl`, повторного SOLID/DRY follow-up, локального app-bundle slice, исправления menu bar bundle identity и двух повторных review 2026-07-04. Он отражает текущий код: macOS event tap, TOML config, отдельный CLI parser, permission checks, raw-input guard, step size, LaunchAgent start at login для CLI, `SMAppService` login item для GUI, JSON startup diagnostics, GUI `.app` bundle, menu bar item и открытые gaps до Scroll Reverser parity.
...
Найдено и статус на момент записи (часть уже исправлена в этом же раунде, часть требует отдельной работы):
...
N105. [Problem] Нет app icon для Finder/System Settings.
N106. [Improve] Создать `.icns` asset и прописать `CFBundleIconFile` в `Info.plist`.
...
N114. [Improve] В `QA.md` явно разделить automated checks и manual checks by human.
...
N117. [Problem] egui UI живет в одном большом `src/ui.rs`.
N118. [Improve] Разбить по SRP: `ui/app.rs`, `ui/settings.rs`, `ui/devices.rs`, `ui/permissions.rs`, `ui/login_item.rs`.
...
N125. [Problem] Restore defaults без confirmation может случайно стереть per-device rules.
N126. [Improve] Добавить confirmation или backup before reset.
...
N139. [Problem] Нет `ROADMAP.md` с маленькими P0/P1/P2 кусками.
N140. [Improve] Свернуть 760 рекомендаций в executable roadmap на 20-30 задач.
...
N145. [Problem] Нет license и privacy docs перед публичным распространением.
N146. [Improve] Добавить `LICENSE`, `PRIVACY.md`, `SECURITY.md` перед release.
N147. [Problem] Нет GitHub Actions.
N148. [Improve] CI: `cargo fmt --check`, `cargo check --no-default-features`, `cargo clippy -- -D warnings`, `cargo test`.
...
N157. [Problem] Нет test для permission-retry UI lifecycle.
N158. [Improve] После runtime split покрыть lifecycle unit tests без egui.
N159. [Problem] Сегодняшние проверки не доказывают реальный scroll на физическом устройстве.
N160. [Improve] Добавить manual QA checklist: mouse wheel, trackpad, Magic Mouse, login, sleep/wake, permissions reset.
...
N162. [Problem] Простого bool-флага недостаточно для UI: он не различает "пытались", "запущено", "заблокировано другим процессом" и "ждет permissions".
N163. [Improve] Ввести `TapLifecycleState` и рисовать status header из него, а не из смеси `enabled`, `tap_error` и permission checks.
...
N165. [Improve] Следующим слоем разделить сам `install_and_run`: `Running`, `AlreadyRunning`, `Stopped`, `Failed(error)`, чтобы это было типизировано глубже runtime.
...
N168. [Problem] При `Restore defaults` можно неожиданно изменить enabled-state и lifecycle без отдельного подтверждения.
N169. [Improve] Сделать reset как action с preview: какие поля будут сброшены, останутся ли device rules.
...
N182. [Problem] Нет кнопки "Open Accessibility" и "Open Input Monitoring" как отдельных действий.
N183. [Improve] Добавить две targeted buttons с fallback на общий Privacy pane.
...
N208. [Problem] Нет design tokens для spacing, colors, status severity.
N209. [Improve] Создать `ui/theme.rs` с spacing scale, status colors and text styles.
...
N224. [Problem] Сборка app bundle не проверяет, что executable остается Mach-O после будущих правок.
N225. [Improve] Добавить `file Contents/MacOS/auto-reverse` smoke в bundle script или release checklist.
N226. [Problem] Bundle script не генерирует app icon.
N227. [Improve] Добавить `assets/AppIcon.iconset` и `iconutil` шаг для `.icns`.
...
N236. [Problem] Нет privacy statement, хотя app слушает input events.
N237. [Improve] Добавить `PRIVACY.md`: local-only, no network, what permissions mean, what is not collected.
N238. [Problem] Нет threat model для event tap/raw input.
N239. [Improve] Добавить `SECURITY.md` с boundaries, no telemetry, reporting process.
N240. [Problem] Нет GitHub Actions, значит regressions легко пропустить перед push.
N241. [Improve] Минимальная CI матрица: fmt, check, check no-default-features, clippy, test.
...
N244. [Problem] `recommendation.md` уже слишком большой для ежедневной работы.
N245. [Improve] Сгенерировать `ROADMAP.md` из N-пунктов: 10 P0, 20 P1, остальное parking lot.
...
N250. [Problem] Нет снимка конкурентных фич Scroll Reverser в одном компактном месте.
N251. [Improve] Обновить parity doc как table: feature, implemented, user-visible, gap, next action.
...
N256. [Problem] Нет "pause for 15 minutes" как ожидаемой menu bar utility функции.
N257. [Improve] Добавить temporary pause с countdown в menu bar.
