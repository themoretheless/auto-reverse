...
T04. **[Done, event-tap latency]** Callback больше не deep-clone-ит `AppConfig` с device rules на каждый event и не ждёт GUI writer: `try_read` fail-open пропускает только конфликтующий event вместо риска CoreGraphics timeout.

T05. **[Done, diagnostics latency]** Debug ring использует `try_lock`; contention не блокирует input callback, а bounded omission считается saturating `AtomicU64`, показывается в Debug Console и сбрасывается явным `Clear`. Poisoned diagnostic lock восстанавливается без panic.

T06. **[Implemented, physical QA open]** `Both` больше не считает отсутствие two-finger callback доказательством Magic Mouse. Положительное one-finger/two-finger evidence различает источники, public scroll phase закрепляет direct session, momentum продолжает последний подтверждённый источник, unknown momentum и ambiguity консервативно используют Trackpad. Нужен физический rapid-alternation pass с обоими устройствами.

T07. **[Done, observability]** Continuous event с coarse `DeltaAxis=0`, но ненулевым `PointDelta`, больше не исчезает из Debug Console/event-rate input: diagnostics читают pixel deltas до и после transform; regression test фиксирует `5 -> -5`.

T08. **[Done, startup ownership]** GUI `SMAppService` registration transactionally удаляет legacy CLI LaunchAgent и откатывается при migration failure; обратное `enable-startup` снимает GUI login item с rollback. Загрузка новой версии также idempotently очищает dual registration, оставшийся от старой версии, а UI после bootout повторяет захват tap.

T09. **[Done, compatibility]** Bundle minimum установлен в macOS 13.0 из-за `SMAppService`. Builder экспортирует тот же `MACOSX_DEPLOYMENT_TARGET`, а smoke сравнивает `LSMinimumSystemVersion` с каждой `LC_BUILD_VERSION`/legacy min-version записью Mach-O.

T10. **[Done, packaging; Finder QA open]** Нестабильная SVG -> `sips` -> `iconutil` цепочка заменена checked-in multi-resolution `assets/AutoReverse.icns` рядом с editable SVG/PNG. Builder и checker валидируют ICNS; bundle, release и install workflow smokes проходят, Quick Look показывает правильную иконку. Финальный Finder/menu-bar visual pass на установленной подписанной копии остаётся manual QA.
...
Реализация `reverse_in_place` изначально трогала три пары полей CGEvent: `DeltaAxis`, `FixedPtDeltaAxis` и `PointDeltaAxis` - независимо считывая и инвертируя каждое. Написав и запустив реальную тестовую программу против настоящего discrete `CGEvent`, выяснилось: запись в `DeltaAxis` заставляет macOS автоматически пересчитать `FixedPtDeltaAxis` и `PointDeltaAxis` **от нового значения**. Поэтому для discrete wheel финальный контракт пишет только `DeltaAxis`. Поздний отдельный regression pass для continuous PIXEL events доказал обратную сторону: их coarse `DeltaAxis` часто равен нулю, а его запись уничтожает исходную pixel precision; входящий continuous event сохраняет coarse field и меняет precise Point/FixedPt representations. Эти два пути намеренно разделены и покрыты разными тестами.
...
- **Magic Mouse vs trackpad**: exclusive public IOHID product inventory теперь имеет приоритет и надёжно закрывает `TrackpadOnly`/`MagicMouseOnly`; timing heuristic включается только для `Both`. Apple прямо предупреждает, что touch objects у gesture events могут неточно отражать реальные касания, поэтому отсутствие двух пальцев больше не считается доказательством Magic Mouse. Быстрое чередование обоих устройств и сторонние smooth wheels остаются heuristic edge cases; unknown/failed probes безопасно используют Trackpad. Точная per-event IOHID attribution по-прежнему относится только к дискретным колёсам.
...
### Magic Mouse classifier: три прохода 2026-07-11, regression 2026-07-13

1. **Research/API audit.** Scroll Reverser действительно использует отдельный `NSEventTypeGesture` tap и 222/333 ms timing, но Apple отдельно предупреждает: touch objects, извлечённые из gesture events, могут неточно отражать активные касания. Реальный Mac подтвердил failure mode: подключён только `Apple Internal Keyboard / Trackpad`, usable two-finger observation отсутствует, а прежнее правило ошибочно выдавало `MagicMouse`.
2. **Implementation/design.** `hid.rs` теперь строит owned snapshot публичных IOHID product names даже для встроенного trackpad без полного VID/PID. Чистый `ContinuousSourceHint` даёт `TrackpadOnly`, `MagicMouseOnly`, `Both` или `Unknown`; matching/removal callbacks публикуют hot-plug изменения через lock-free atomic. Exclusive hint всегда побеждает, старая timing/momentum policy применяется только к `Both`, а unknown/failed passive tap использует Trackpad. UI сохраняет три независимых live checkbox без platform types в domain layer.
3. **Review/verification.** Regression tests фиксируют exclusive trackpad без touch observations, Magic Mouse без gesture monitor, `Both` timing transitions и unknown fallback. Живые `devices`/`doctor` на macOS 26.6 печатают `trackpad only` для текущего набора hardware; поэтому `reverse_trackpad`, а не `reverse_magic_mouse`, управляет встроенным тачпадом. Физический сценарий с одновременно подключёнными Magic Mouse и trackpad остаётся manual QA.

Исходники исследования: [Scroll Reverser `MouseTap.m`](https://github.com/pilotmoon/Scroll-Reverser/blob/187bf3945b6107cd8486327c6165f32e523535a4/MouseTap.m), [Apple Handling Trackpad Events](https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/EventOverview/HandlingTouchEvents/HandlingTouchEvents.html), [Apple `NSEvent`](https://developer.apple.com/documentation/appkit/nsevent).
...
N218. [Done] Главный Magic Mouse parity gap закрыт public inventory-first best-effort classifier.
...
N227. [Superseded by T10] Editable `assets/AppIcon.svg` сохранён, но runtime conversion удалён; builder копирует проверенный multi-resolution `.icns`.
...
N324. [Superseded by T10] Полный iconset сохранён внутри checked-in `assets/AutoReverse.icns`; build больше не зависит от несовместимой SVG/iconutil conversion chain.
...
N355. [Done] Magic Mouse and trackpad have separate live paths through public IOHID inventory plus a `Both` gesture heuristic.
...
R26. [Done] macOS adapter ставит/читает 64-bit `AUTORVRS` marker через public `kCGEventSourceUserData`; normalized event получает `synthetic=true`, а scheduler sample требует `AutoReverseSynthetic` tag до появления platform poster.
R27. [Done] Каждый `ScheduledSample` несет vertical/horizontal session generation, unique wake id, scheduled/creation/expiry timestamps и due-anchored 8 ms TTL; late callback не получает новый TTL, stale generation, replaced wake и expired sample имеют отдельные deterministic tests.
R28. [Done] Pure scheduler хранит только optional wake lease: Off/empty output остается Idle, последний tail удаляет wake, а поздний idle callback ничего не эмитит; timer/thread намеренно отсутствуют.
R29. [Done] Dynamics или scheduler fault очищает pending state/wake, точно возвращает текущий original delta и latch-ит fail-open для следующих событий до explicit reset; counters используют checked overflow, wake id не переиспользуется после reset, healthy reset является no-op.
R30. [Done] `BenchmarkTransfer::ScreenHeightHypothesis` масштабирует только внутри explicit benchmark trial и записывается в result/CSV; `Baseline` остается default, runtime/config не зависят от hypothesis.
...
## Config durability and manual updates: S01-S10 (2026-07-18)

S01. [Done] Atomic config save теперь синхронизирует содержимое unique temporary file до rename, поэтому успешная запись не зависит только от page cache.
S02. [Done] После rename синхронизируется parent directory, а каждый новый committed config создается с Unix mode `0600`; оба свойства проверяются в isolated tests.
S03. [Done] `ConfigStore::inspect_existing` читает и валидирует существующий TOML без создания parent directory, config или persistent lock.
S04. [Done] `validate-config --text|--json` различает `valid`, `missing`, `invalid` и I/O `error`; invalid report завершается ненулевым status и сохраняет config byte-for-byte.
S05. [Done] `repair-config` - единственная явная destructive recovery command: missing config получает defaults, valid остается byte-for-byte неизменным, invalid сохраняется рядом полностью.
S06. [Done] Quarantine использует atomically exclusive hard link вместо `exists + rename`, синхронизирует backup до удаления original path и восстанавливает исходник при любой ошибке replacement.
S07. [Done] `AppError::code` дает стабильные coarse identifiers (`E_IO`, config parse/validation/serialization/change, trace, permission, platform, usage), а CLI печатает код отдельно от изменяемого human text.
S08. [Done] Update policy выбрана явно: приложение не делает background/version network requests, а только по пользовательскому действию передает canonical release URL системному браузеру.
S09. [Done] `open-releases` поддерживает взаимоисключающие `--latest` и `--all`; без override использует beta preference только как выбор manual destination.
S10. [Done] Advanced tab получил компактный About & Updates block с версией, Latest release, All releases и inline launch error без карточек, updater toggle или ложного обещания automatic update.

Источники решения: [GitHub canonical release links](https://docs.github.com/en/repositories/releasing-projects-on-github/linking-to-releases), [Sparkle security and appcast requirements](https://sparkle-project.org/documentation/), [Apple notarization workflow](https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution). Automatic in-app updater остается отдельной будущей возможностью только после Developer ID/notarization, signed appcast/EdDSA и release-channel rollback design; отсутствие этих prerequisites не маскируется работающей кнопкой.
