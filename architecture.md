...
- `src/ui.rs` - egui settings window (feature `gui`, включён по умолчанию). Открытие окна также автозапускает `run` (детач-процесс того же бинарника), если `config.enabled=true`, permissions уже выданы и `daemon_lock` не видит живого daemon-а.
...
- `src/platform/macos/` - вся OS-специфика и unsafe-код в одном месте: `scroll_events.rs` (маппинг полей CGEvent: прочитать событие, записать решение), `permissions.rs` (Accessibility + Input Monitoring TCC), `hid.rs` (IOHIDManager wheel monitor: атрибуция дискретного скролла конкретному vendor/product ID для `device_rules`), `startup.rs` (LaunchAgent автозапуск), `event_tap.rs` (runtime-цикл CGEventTap, recovery при disabled tap; пишет pid file через `daemon_lock` перед входом в run loop), `daemon_lock.rs` (pid-file lock: `write_pid_file`/`running_daemon_pid`, гарантирует, что никогда не запущено два `run`-daemon-а одновременно - ручной, LaunchAgent и GUI-автозапуск все проверяют один и тот же файл).
- `Cargo.toml` - `core-foundation`/`core-graphics` теперь target-specific dependencies: чистое ядро собирается без них.
- `scripts/build-app-bundle.sh` - локальный `.app` bundle для macOS Privacy & Security; двойной клик открывает settings window (`ui`), которое теперь также автозапускает `run` (см. выше).
...
src/
  ...
  platform/
    ...
    macos/
      ...
      daemon_lock.rs               pid-file lock: only one `run` daemon at a time
scripts/
