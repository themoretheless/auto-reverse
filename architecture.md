...
- `src/platform/macos/` - вся OS-специфика и unsafe-код: `scroll_events.rs` владеет полями CGEvent, `permissions.rs` - TCC, `hid.rs` - IOHIDManager inventory, serial/location/public transport reads и timestamped `WheelSnapshot`, `gesture.rs` - отдельный listen-only public AppKit tap и перевод touch/momentum observations в чистый classifier, `event_tap.rs` - lock/install/readiness/run loop, confidence-gated HID-source normalization и lifetime-safe registry обоих tap ports, `tap_metrics.rs` - bounded on-demand `CGGetEventTapList`, `external_url.rs` - только trusted release destinations через `/usr/bin/open`, остальные adapters сохраняют прежние узкие ответственности.
...
- `recommendation.md` - 970 пунктов backlog/review (500 базовых + N01-N400 + R01-R60 + S01-S10); `ROADMAP.md` сворачивает их в маленькие исполнимые batches.
...
и завершенный config/update pass `S01-S10` (970 всего). Блок ниже зеркалит
...
- repeated review keeps exactly 500 base backlog items, later extended by N01-N400 through startup, menu bar, lifecycle, design, pause, icon, CI and documentation passes, R01-R60 after the external research pass, and S01-S10 for config durability/manual updates (total 970);
