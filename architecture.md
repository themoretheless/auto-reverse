...
- `src/scroll_benchmark.rs` - чистые matrix bounds и ScrollTest-style state machine: Known/Unknown, 66 ms settle, movement time, switchbacks и maximum overshoot.
...
- `DESIGN.md`, `QA.md`, `PRIVACY.md`, `SECURITY.md`, `RELEASE.md`, `TRACE.md`, `BENCHMARK.md`, `CONTRIBUTING.md` - отдельные контракты вместо смешивания release/design/measurement требований с architecture narrative; `RELEASE.md` является единственным полным release checklist.
...
Measurement boundaries уже реализованы; dynamics/profile модули создаются
только когда начинается соответствующая итерация:
