...
- `src/cli.rs` - маленький parser команд и флагов: `run`, `ui`, `doctor --no-create`, `init`, `enable`, `disable`, `toggle`, `enable-startup`, `disable-startup`, `startup-status --json`, internal-safe `prepare-uninstall`, `devices`, `config-path`, `show-config`, `simulate` и `trace-lab`.
...
- `src/scroll_lab.rs` - pure magnitude/interval/direction/duration/clutch metrics, axis totals и обязательный constant-gain baseline.
...
Первый measurement boundary уже реализован; остальные создаются только когда
начинается соответствующая итерация:
...
src/scroll_dynamics.rs                    pure discrete-wheel dynamics
src/config/profiles.rs                    profile inheritance and precedence
src/platform/macos/tap_metrics.rs         CGGetEventTapList adapter
