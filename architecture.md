...
- `src/config/` - разделен по ответственности: `schema.rs` хранит поля/defaults/validation, `device_rules.rs` - чистый matching и мутацию для UI/tray, `profiles.rs` - field-by-field precedence serial > location > VID/PID > device kind > global, `store.rs` - пути, TOML I/O, atomic save через уникальный temp file, persistent cross-process lock, exact revision snapshots/CAS и locked `update`. Все profile values живут в существующем `device_rules`, второй profile database нет. `mod.rs` реэкспортирует стабильный facade.
...
- `src/platform/macos/` - вся OS-специфика и unsafe-код: `scroll_events.rs` владеет полями CGEvent, `permissions.rs` - TCC, `hid.rs` - IOHIDManager inventory, serial/location/public transport reads и единый `WheelSnapshot`, `gesture.rs` - отдельный listen-only public AppKit tap и перевод touch/momentum observations в чистый classifier, `event_tap.rs` - lock/install/readiness/run loop, HID-source normalization и lifetime-safe registry обоих tap ports, `tap_metrics.rs` - bounded on-demand `CGGetEventTapList`, остальные adapters сохраняют прежние узкие ответственности.
