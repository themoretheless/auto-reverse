...
- `src/main.rs` - тонкий CLI entrypoint: `run`, `ui`, `doctor`, `init`, `enable`, `disable`, `toggle`, `enable-startup`, `disable-startup`, `startup-status`, `devices`, `config-path`, `show-config`, `simulate`.
