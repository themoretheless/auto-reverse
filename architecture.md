...
- `src/cli.rs` - маленький parser команд и флагов: `run`, `ui`, `doctor --no-create`, `init`, `enable`, `disable`, `toggle`, `enable-startup`, `disable-startup`, `startup-status --json`, internal-safe `prepare-uninstall`, `devices`, `config-path`, `show-config`, `simulate` (включая `--vendor-id`/`--product-id` и optional `--serial-number`/`--location-id`).
...
- `src/device_classifier.rs` - чистая state machine без AppKit: discrete = mouse, свежий two-finger signal = trackpad, другой continuous = Magic Mouse-like; momentum наследует последний continuous source, а отдельный conservative fallback сохраняет прежнее поведение.
...
- `src/platform/macos/` - вся OS-специфика и unsafe-код: `scroll_events.rs` владеет полями CGEvent, `permissions.rs` - TCC, `hid.rs` - IOHIDManager, serial/location reads и единый `WheelSnapshot`, `gesture.rs` - отдельный listen-only public AppKit tap и перевод touch/momentum observations в чистый classifier, `event_tap.rs` - lock/install/readiness/run loop и lifetime-safe registry обоих tap ports, остальные adapters сохраняют прежние узкие ответственности.
...
- `scripts/` - `build-app-bundle.sh` строит `.app` с реальным Mach-O, Retina icon, versioned plist и ad-hoc подписью; `check-app-bundle.sh` разделяет strict и identity-only validation; `install-app-bundle.sh` делает staged same-volume install/update с rollback; `uninstall-app-bundle.sh` снимает startup registrations и удаляет только проверенный bundle; `check-install-workflow.sh` гоняет всё в isolated temp destination.
...
Magic Mouse и trackpad теперь различаются live через публичную эвристику: пассивный session tap получает `NSEventTypeGesture`, AppKit дает число touching fingers, а чистая state machine сопоставляет этот сигнал с continuous/momentum scroll. Отдельный пассивный tap выбран намеренно, чтобы не вмешиваться в Notification Center и shake-to-locate gestures. Это не физическая identity: быстрое чередование устройств и сторонние smooth wheels остаются heuristic edge cases, а если tap не создается, continuous scroll снова безопасно считается trackpad. Физическая Magic Mouse/trackpad проверка остаётся ручной QA. Для дискретных колёс identity v2 использует serial или port/location fallback; reconnect/смена порта/две одинаковые мыши тоже остаются ручной QA.
...
src/
  ...
  device_classifier.rs             pure gesture/timing classifier + fallback
...
scripts/
  ...
  check-app-bundle.sh              strict/identity-only bundle validation
...
start
  ...
  -> passive gesture tap counts touching fingers without changing the event
  -> classify source from continuity + momentum + recent two-finger observation
  -> if passive tap is unavailable: classify continuous scroll as trackpad
...
- no Developer ID/hardened-runtime/notarized signing yet.
