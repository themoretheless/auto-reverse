...
- `src/config/` - разделен по ответственности: `schema.rs` (какие настройки ЕСТЬ: поля, defaults, validation, per-device policy) и `store.rs` (где они ЖИВУТ: пути, TOML I/O, atomic save через уникальный temp file, persistent cross-process lock, exact revision snapshots/CAS и locked `update`). `mod.rs` реэкспортирует стабильный facade, так что вызывающий код не зависит от внутреннего разбиения.
...
- `src/platform/macos/` - вся OS-специфика и unsafe-код: `scroll_events.rs` владеет полями CGEvent, `permissions.rs` - TCC, `hid.rs` - IOHIDManager и единый `WheelSnapshot` с разделяемым `Arc<str>` имени, `event_tap.rs` - lock/install/readiness/run loop, `TapRunOutcome` и lifetime-safe tap-port registry, `power_events.rs` - NSWorkspace sleep/wake observer, `debug_log.rs` - bounded structured diagnostics (`DecisionReason` + raw source fields), `daemon_lock.rs` - `run.lock`/`ui.lock`, `startup.rs`/`login_item.rs` - два start-at-login пути, `quit_handler.rs` - tray-only quit contract, `tray.rs` - AppKit adapter. Чистая мутация quick-pick rules уже вынесена в `tray/device_rules.rs`.
