...
- `src/ui.rs` - egui settings window (feature `gui`, включён по умолчанию), слитый с event tap в один процесс: открытие окна запускает `CGEventTap` на фоновом потоке (`event_tap::install_and_run`), деля один `Arc<RwLock<AppConfig>>` между окном и потоком - изменение любого чекбокса применяется к следующему scroll-событию немедленно, без рестарта. `install_and_run` сам берет `daemon_lock` первым делом, так что headless `run` и этот in-process поток никогда не держат живой tap одновременно. Иконка в menu bar (`platform::macos::tray`) живет всё время процесса; это native AppKit template status icon, поэтому AppKit сам подбирает контраст для светлой/темной строки меню. Закрытие окна (красная кнопка/Cmd-W) перехватывается через `ViewportCommand::CancelClose` и прячет окно, а не завершает процесс - только Quit из трея реально вызывает `std::process::exit`. "Start at login" в окне использует `platform::macos::login_item` (`SMAppService.mainAppService()`), отдельно от CLI-механизма `startup.rs`/`enable-startup`.
