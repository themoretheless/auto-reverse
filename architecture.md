...
Auto Reverse - системная Rust-утилита для reverse scrolling в стиле Scroll Reverser. Проект уже не scaffold: есть macOS event tap, TOML-конфиг, CLI, отдельный parser команд, rule resolver, step size, permission checks, raw-input guard, взаимно исключаемые GUI/CLI login owners и автоматизированные tests/workflow smokes.
...
- `src/device_attribution.rs` - чистая bounded policy для last-active HID correlation: high до 8 ms, medium до 50 ms, затем timeout; только high/medium разрешают передать identity в scroll policy.
- `src/device_catalog.rs` - чистая presentation projection, объединяющая повторные HID services и разделяющая Connected, Remembered и Unavailable; alias и duplicate suffix вычисляются без AppKit/egui.
- `src/device_test.rs` - чистая пятисекундная state machine exact-device проверки. Она принимает только prefiltered physical discrete activity и требует полного совпадения `DeviceIdentity`; UI не обещает exact test для model-wide VID/PID.
- `src/app_session.rs` - pure non-live contract для будущих app rules: target PID и generation закреплены на direct/momentum session, а stale/orphaned momentum fail-open не принимает новый target.
- `src/input_policy.rs` - единый pure provenance/bypass resolver для hardware, posted process, self-synthetic, virtual и unknown HID; его одновременно используют transform и diagnostics.
- `src/settings_search.rs` - pure bounded fuzzy index настроек и Diagnostics destinations; egui отвечает только за field/results/navigation.
- `src/update_policy.rs` - pure network-free contract canonical release URLs и stable/prerelease manual channel; legacy flags не могут включить background I/O.
- `src/preset_preview.rs` - pure 15-second pending/confirm/expire policy; preview никогда сам не мутирует config.
- `src/refresh_policy.rs` - pure coalescing AppKit/IOHID/wake signals и tolerant 30-second backstop без missed-tick replay.
- `src/tap_watchdog.rs` - pure public-state health policy: 1-second samples, two-sample hysteresis, three-attempt budget и three-healthy reset.
- `src/device_classifier.rs` - чистая state machine без AppKit: discrete = mouse, exclusive IOHID inventory фиксирует Trackpad/Magic Mouse, positive one-finger/two-finger evidence и public scroll phase закрепляют source только при реальном `Both`; missing/unknown evidence и unknown momentum безопасно становятся Trackpad.
- `src/device_source.rs` - чистая классификация public HID transport: `Virtual` и observed unknown дают fail-open pass-through; `NotObserved` сохраняет kind fallback.
- `src/input.rs` - нормализованный `ScrollEvent` с `source_pid`, `HidSourceClass` и optional shared `Arc<DeviceIdentity>`.
- `src/diagnostics.rs` - чистый общий словарь axis/category/decision reason, используемый Debug Console и trace без зависимости от macOS. Debug event дополнительно хранит attribution, classifier evidence, input provenance, resolved profile и process-local exact identity для device test; row hover/search/detailed CSV объясняют chain, но serial/location не сериализуются.
- `src/diagnostics_summary.rs` - privacy-by-construction clipboard summary: typed input содержит только config, redacted runtime enum и aggregate device/reason vocabulary, поэтому raw delta/time/PID/name/app/window передать formatter-у невозможно.
- `src/recovery_audit.rs` - pure bounded журнал восстановления: wake, точные event-tap disable reasons, watchdog и permission loss имеют независимые attempt counters; process-local adapter и Debug Console не сохраняют PID, device identity или wall time.
...
- `recommendation.md` - 980 пунктов backlog/review (500 базовых + N01-N400 + R01-R60 + S01-S10 + T01-T10); `ROADMAP.md` сворачивает их в маленькие исполнимые batches.
...
- `DESIGN.md`, `QA.md`, `PRIVACY.md`, `SECURITY.md`, `RELEASE.md`, `UPDATES.md`, `TRACE.md`, `BENCHMARK.md`, `DYNAMICS.md`, `CONTRIBUTING.md` - отдельные контракты вместо смешивания release/design/measurement требований с architecture narrative; `RELEASE.md` является единственным полным release checklist, `UPDATES.md` - единственным update-policy контрактом.
- `scroll-reverser-parity.md` - parity-чеклист Scroll Reverser.

Magic Mouse и trackpad различаются inventory-first: публичные IOHID product names сначала определяют `TrackpadOnly`, `MagicMouseOnly`, `Both` или `Unknown`, а matching/removal callbacks обновляют атомарный hint при hot-plug и sleep/wake. Exclusive hint не зависит от touch-count; только `Both` использует пассивный `NSEventTypeGesture` tap. Positive one-finger/two-finger observation выбирает источник, public scroll phase закрепляет direct session, valid momentum продолжает последний источник. Отсутствующий callback и unknown momentum не доказывают Magic Mouse и дают Trackpad fallback. Это исправляет регрессию, где trackpad мог подчиняться checkbox Magic Mouse. Быстрое чередование обоих устройств и сторонние smooth wheels остаются physical/manual QA. Для дискретных колёс identity v2 использует serial или port/location fallback.
...
## Config transfer и локальная диагностика (R46-R50)

Import - двухфазная операция. `config::transfer` сначала открывает выбранный
regular file, ограничивает чтение 256 KiB, отклоняет symlink/world-writable
mode, открывает с `O_NOFOLLOW` и проверяет dev/inode/length/mtime до и после
чтения. Только после этого
UTF-8 попадает в TOML parser. Unversioned и explicit-v0 документы получают
детерминированный migration report и current defaults; current v1 проходит без
миграции, future version отклоняется до попытки потерять неизвестные поля.

Вторая фаза не пишет файл: pure preview сравнивает candidate с текущим config
по General, Devices, Startup и Advanced и возвращает только отличающиеся поля.
Пока review открыт, он rebases на текущую in-memory конфигурацию. Apply копирует
только показанные sections, затем использует тот же `ConfigStore::save_if_unchanged`,
что и обычный GUI. Поэтому импорт не создаёт отдельный writer и не обходит
защиту от concurrent CLI/tray save.

Clipboard summary и device test используют разные минимальные projections
одного process-local ring buffer. Summary получает только enums и counts, а
значит не имеет доступа к names, serial/location, PID, timestamps, deltas и
target app/window context. Device test, наоборот, получает exact identity, но
никогда её не сериализует: после нажатия засчитывается только последующее
physical discrete hardware event с тем же serial/location. Для model-wide
VID/PID точность недоказуема, поэтому UI показывает unavailable state без
кнопки вместо ложного успеха.

## Preview, reset и runtime recovery (R51-R55)

Dynamics picker не пишет shared config при выборе. Pure `PresetPreview`
держит committed/candidate/deadline 15 секунд; только `Use preset` возвращает
значение coordinator-у для обычного CAS save. Expiry, Revert или внешний
config change оставляют live runtime неизменным. Две progress bars показывают
параметры модели честно: platform scheduler всё ещё отсутствует.

Reset scopes реализованы в `config/reset.rs`: exact-device reset удаляет только
preferred selector, dynamics reset меняет step/preset fields и сохраняет
direction/alias, Restore all использует полный default config и после успешного
save снимает pause и unregister-ит GUI login item. Ни один scoped reset не
создаёт второй storage path.

Permission state кешируется. `NSApplicationDidBecomeActive`, NSWorkspace wake
и IOHID match/removal generation дают дешёвые сигналы; `RefreshPolicy` запускает
полную проверку по событию и только раз в 30 секунд как backstop. Disabled
launch делает read-only TCC check и не показывает consent prompt. Tray читает
тот же `Arc<AtomicBool>`.

`event_tap::enabled_state` вызывает публичный `CGEventTapIsEnabled` под mutex,
который одновременно защищает lifetime CFMachPort. `TapWatchdog` не действует
после одного sample: два Disabled/NotInstalled observation дают rearm/restart,
не более трёх раз за episode. Attempt budget сбрасывается только после трёх
healthy samples или явного Retry; exhausted state виден в UI и diagnostics.

## Recovery audit, fixtures и release gates (R56-R60)

`RecoveryAudit` отделён от watchdog policy и от macOS FFI. Он хранит не более
64 process-local записей, считает попытки отдельно для wake, точного
`TapDisabledByTimeout`, `TapDisabledByUserInput`, watchdog-disabled и permission
loss и отдаёт Debug Console только sequence/reason/action/attempt. Watchdog не
выдаётся за источник timeout: callback event tap остаётся единственным местом,
где известна точная disable reason.

Migration coverage закреплён файлами `tests/fixtures/config/`: совокупность
исторических v0 fixtures покрывает все поддерживаемые root/device-rule keys,
а отдельный каталог опечаток проверяет каждый известный плохой key. Unknown
field не удаляется молча: parser возвращает ошибку до preview/save, поэтому
исходный документ остаётся неизменным. Пять deterministic property suites по
512 seeds проверяют trace parsing/round-trip, generated trace shapes, config
fuzz rejection, v0/current migration round-trip и two-axis conservation/idle.

`QA.md` теперь содержит стабильные PR-01..PR-06 для Safari zoom, Launchpad,
Catalyst/iOS apps, Universal Control, iPhone Mirroring и remote desktop.
Release smoke проверяет наличие и уникальность строк; пустой Result честно
означает, что физический прогон ещё не выполнен. Production release требует
build, `Pass` и evidence/tester во всех шести строках до build/notary действий.

Dynamics остаётся default-off. `AUTO_REVERSE_DISABLE_DYNAMICS` fail-closed,
malformed value также выключает experiment. Source constant и release manifest
должны совпадать; default-on запрещён без всех шести physical classes, минимум
30 sessions/class, p95 regression <= 5%, scheduler tail <= 8 ms и нуля
fail-open violations. `rollback-dynamics` атомарно очищает только smooth preset
в global/device rules, сохраняя direction, alias, step size и прочий config.
...
src/
  ...
  device_attribution.rs           bounded last-active confidence policy
  device_catalog.rs               connected/remembered/unavailable projection
  device_source.rs                public HID transport fail-open policy
  device_test.rs                  pure exact-identity recent-event test state
  input_policy.rs                 shared input provenance and bypass resolver
  diagnostics.rs                  stable axis/category/reason vocabulary
  diagnostics_summary.rs          aggregate privacy-bounded clipboard text
  app_session.rs                  future app-target session pin, non-live
  settings_search.rs              fuzzy settings/diagnostics index
  preset_preview.rs               temporary preset confirm/expiry policy
  refresh_policy.rs               notification coalescing + timer backstop
  tap_watchdog.rs                 bounded event-tap health/recovery policy
  recovery_audit.rs               bounded typed recovery reasons/attempts
  dynamics_gate.rs                fail-closed kill switch + acceptance policy
  ...
  ui/
    ...
    device_rules.rs                catalog/profile/test/reset device rows
    preset_preview.rs              compact non-live model preview controls
    theme.rs                       handoff tokens and custom egui controls
    local_export.rs                shared CSV escaping + atomic local replace
    config_transfer.rs             native panels + pending section review
  ...
  config/
    ...
    profiles.rs                    field inheritance and fixed precedence
    reset.rs                       exact-device and dynamics reset scopes
    store.rs                       TOML I/O, atomic save, file lock, snapshots/CAS
    transfer/
      mod.rs                       public transfer facade and typed errors
      document.rs                  version/schema migration + TOML
      diff.rs                      pure section review/application
      secure_file.rs               bounded O_NOFOLLOW file trust boundary
  platform/
    ...
    macos/
      ...
      recovery_log.rs              process-local adapter to pure recovery audit
      ...
      save_panel.rs                native config/diagnostic panels + Finder reveal (gui only)
...
scripts/
  ...
  check-dynamics-release-gate.sh   source/manifest/threshold parity gate
  check-regression-matrix.sh       stable PR-01..PR-06 QA structure gate
...
packaging/
  ...
  dynamics-release-gate.toml       exact-candidate dynamics evidence manifest
tests/
  cli_integration.rs               binary smoke в isolated HOME/config paths
  property_invariants.rs           five deterministic 512-seed property suites
  fixtures/config/                 historical and invalid-key migration inputs
```
...
start
  ...
  -> config import securely reads and migrates into an in-memory candidate, then renders a section diff without writing
  -> confirmed import copies only reviewed sections and enters the same exact-revision save path
  ...
  -> classify `Both` from positive one-finger/two-finger evidence
  -> pin direct source through public Began/Changed/Ended/Cancelled scroll phases
  -> continue only known momentum; unknown/missing evidence falls back to Trackpad
  ...
  -> for gui builds: record reason enum + raw source/device fields and local exact identity in the in-memory ring buffer
  -> device test accepts only a later physical discrete event with the same serial/location identity
  -> clipboard summary projects the buffer to aggregate enums/counts before formatting
...
second ui launch
  ...
  -> if another process owns it: atomically publish PID + ReloadAndOpen and exit successfully
  -> ordinary CLI mutation publishes ReloadOnly without changing window visibility
...
enable-startup
  ...
  -> unregister an existing GUI SMAppService owner; roll back LaunchAgent on failure
  -> set config.start_at_login = true
  -> report whether LaunchAgent points at this binary

GUI Start at Login
  -> register SMAppService.mainAppService()
  -> remove and boot out the legacy CLI LaunchAgent
  -> roll back a fresh GUI registration if legacy removal fails
  -> retry the in-process tap after the prior owner releases run.lock

scripts/build-app-bundle.sh
  -> cargo build
  -> create target/<profile>/Auto Reverse.app
  -> copy auto-reverse into Contents/MacOS
  -> validate and copy checked-in assets/AutoReverse.icns
  -> export MACOSX_DEPLOYMENT_TARGET >= 13.0
  -> write matching versioned Info.plist with LSUIElement=true and CFBundleIconFile
  ...
  -> check Mach-O/plist minimum parity, icon, identifier and signature via check-app-bundle.sh
...
3. Profiles/UX: optional fields, tri-state direction, aliases и device-state
   catalog реализованы в существующем `DeviceRule`; session-pinned app-policy
   contract, config import dry-run, settings search, exact device test,
   temporary preset preview, scoped resets, notification refresh и bounded
   public-state watchdog также завершены. R56-R60 добавили typed recovery
   audit, exhaustive migration fixtures, deterministic property suites,
   platform regression matrix и fail-closed dynamics release gate.

Measurement, pure dynamics/scheduler contracts и profile/source boundaries уже
реализованы. Platform scheduler по-прежнему не создан и не меняет live
scrolling:
...
src/scroll_dynamics.rs                    continuous bypass/two-axis facade [done, not live]
src/scroll_dynamics/axis.rs               session/cancellation/distance ledger [done]
src/scroll_dynamics/rate.rs               bounded dt and recent-rate window [done]
src/scroll_dynamics/preset.rs             stable preset parameters [done]
src/scroll_scheduler.rs                   fail-open orchestration [done, not live]
src/preset_preview.rs                     temporary confirm/expiry policy [done]
src/refresh_policy.rs                     notification/backstop policy [done]
src/tap_watchdog.rs                       bounded enabled-state recovery [done]
src/recovery_audit.rs                     typed bounded recovery audit [done]
src/dynamics_gate.rs                      runtime/release kill gate [done, dynamics not live]
src/scroll_scheduler/schedule.rs          wake/generation/TTL contract [done]
src/config/profiles.rs                    profile inheritance and precedence [done]
src/device_source.rs                      pure public HID source policy [done]
src/device_attribution.rs                 bounded HID correlation confidence [done]
src/device_catalog.rs                     connected/remembered/unavailable projection [done]
src/app_session.rs                        future target PID session pin [done, not live]
src/input_policy.rs                       shared provenance and bypass [done]
src/settings_search.rs                    bounded fuzzy navigation [done]
src/config/transfer/                      migration/diff/secure import split [done]
src/diagnostics_summary.rs                aggregate-only clipboard text [done]
src/device_test.rs                        exact recent-event state machine [done]
src/platform/macos/tap_metrics.rs         CGGetEventTapList adapter [implemented]
src/platform/macos/recovery_log.rs        process-local recovery audit adapter [done]
...
- versioned config export/import with migration report and secure dry-run.
...
continuity and physical-device scenarios remain release QA. Update policy is
resolved as manual browser navigation with no application network client; a
future automatic updater is a separate signed-release project.
...
- reviewed v0/bad-key migration fixtures and non-destructive unknown-field
  rejection (done);
- automatic in-place store migration (still intentionally separate);
- packaging and signing;
- automatic updater prerequisites and rollback design, only if manual releases
...
## S01-S10: Config и Updates

| ID | Status | Маленький самостоятельный кусок |
| --- | --- | --- |
| S01 | Done | `sync_all` временного config до atomic rename. |
| S02 | Done | Parent-directory sync и committed mode `0600`. |
| S03 | Done | Read-only inspect без создания directory/config/lock. |
| S04 | Done | `validate-config` с text/JSON status и nonzero invalid result. |
| S05 | Done | Explicit `repair-config` с exact sibling backup. |
| S06 | Done | Exclusive quarantine и rollback original при failed replacement. |
| S07 | Done | Stable CLI error codes, включая config и permission families. |
| S08 | Done | Manual update policy без application-owned network requests. |
| S09 | Done | Canonical latest/all release channels в CLI и pure policy. |
| S10 | Done | Compact egui About & Updates block с inline launch error. |

`config/store.rs`, `error.rs`, `update_policy.rs` и
`platform/macos/external_url.rs` сохраняют SRP: storage не знает UI/браузер,
pure policy не импортирует AppKit, а egui только вызывает typed action.

## Top 500 mirror

`recommendation.md` остаётся canonical source: там 500 базовых предложений,
проблем, ошибок и улучшений, follow-up `N01-N400`, research backlog `R01-R60`,
завершенный config/update pass `S01-S10` и hardening pass `T01-T10` (980
всего). Блок ниже зеркалит
...
- repeated review keeps exactly 500 base backlog items, later extended by N01-N400 through startup, menu bar, lifecycle, design, pause, icon, CI and documentation passes, R01-R60 after the external research pass, S01-S10 for config durability/manual updates, and T01-T10 for the four-agent runtime/classifier/packaging pass (total 980);
...
- 2026-07-18 R46-R50 review added typed config migration/diff, bounded secure
  import, aggregate clipboard diagnostics and exact local device testing. Its
  adversarial pass removed caller-supplied outcome categories, made clipboard
  confirmation transient, checked same-size source mutation, and disabled
  exact testing when only model-wide VID/PID is known.
- 2026-07-18 R51-R55 three-pass review added temporary preset confirmation,
  three reset scopes, disabled-state permission quietness, notification-led
  refresh and a public-state event-tap watchdog. Review moved device UI into
  its own module, made Restore all transactional before login-item removal,
  surfaced watchdog exhaustion and bounded automatic recovery to three tries.
- 2026-07-18 R56-R60 three-pass review added independent recovery reasons and
  attempt counts, complete v0/bad-key fixtures, five 512-seed property suites,
  a stable six-scenario platform matrix, and a fail-closed dynamics gate with
  atomic CLI rollback. Review restricted permission audit to enabled runtime,
  checked fixture field coverage, renamed unconfirmed restart outcomes,
  removed threshold drift, and made blank physical results block production.
- 2026-07-18 S01-S10 three-pass review added durable private config commits,
  side-effect-free validation, exact explicit repair/rollback, stable CLI error
  codes and a manual no-background-network release policy. The adversarial pass
  reclassified invalid UTF-8 as config validation, replaced racy
  `exists + rename` quarantine with exclusive hard-link preservation, and made
  the browser adapter verify `/usr/bin/open` exit status.
- 2026-07-19 menu-visibility review wired `show_menu_bar_icon` to the live
  `NSStatusItem`, added Advanced and CLI recovery, and preserved one GUI owner.
...
- import supports reviewed v0->v1 migration and exhaustive fixtures, but
  automatic in-place store migration remains intentionally absent;
- PR-01..PR-06 have enforced matrix structure, but their exact stapled-bundle
  manual results are blank until physical platform QA is performed;
- dynamics has a kill switch, rollback and release thresholds, but remains
  disconnected from live input and has no accepted physical evidence;
