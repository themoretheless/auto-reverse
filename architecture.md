...
- `src/platform/macos/` - вся OS-специфика и unsafe-код в одном месте: `scroll_events.rs` (маппинг полей CGEvent: прочитать событие, записать решение), `permissions.rs` (Accessibility + Input Monitoring TCC), `event_tap.rs` (runtime-цикл CGEventTap, recovery при disabled tap).
