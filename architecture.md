...
- `src/diagnostics.rs` - чистый общий словарь axis/category/decision reason, используемый Debug Console и trace без зависимости от macOS. Debug event дополнительно хранит attribution, classifier evidence, input provenance и resolved profile; row hover/search/detailed CSV объясняют chain, privacy trace его не сериализует.
...
3. Profiles/UX: optional fields, tri-state direction, aliases и device-state
   catalog реализованы в существующем `DeviceRule`; дальше идут session-pinned
   app-policy research, config import dry-run, settings search и device test row.
