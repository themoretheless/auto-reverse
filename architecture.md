...
3. Profiles/UX: optional fields, tri-state direction, aliases и device-state
   ...
   contract, config import dry-run, settings search и exact device test row
   также завершены. Следующий пакет отделяет preview/reset scopes и runtime
   notification/watchdog policy.
