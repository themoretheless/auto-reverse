...
- `src/ui.rs` - egui settings window (feature `gui`, включён по умолчанию), слитый с event tap в один процесс: открытие окна запускает `CGEventTap` на фоновом потоке (`event_tap::install_and_run`), деля один `Arc<RwLock<AppConfig>>` между окном и потоком - изменение любого чекбокса применяется к следующему scroll-событию немедленно, без рестарта. Если приложение открыли до выдачи Accessibility/Input Monitoring, UI оставляет старт tap pending и автоматически повторяет запуск, когда оба permission-check становятся зелеными. `install_and_run` сам берет `daemon_lock` первым делом, так что headless `run` и этот in-process поток никогда не держат живой tap одновременно. Отдельный `ui.lock` не дает открыть второе окно/вторую menu-bar иконку, даже когда tap не стартует. Settings window приведён к дизайн-handoff: segmented tabs General/Devices/Permissions, pinned status/master toggle, compact native rows, custom checkboxes/buttons/slider/chips, Debug Console viewport с search/filter/export/clear. Закрытие окна (красная кнопка/Cmd-W) перехватывается через `ViewportCommand::CancelClose` и прячет окно, а не завершает процесс - только Quit из трея реально вызывает `std::process::exit`. "Start at login" в окне использует `platform::macos::login_item` (`SMAppService.mainAppService()`), отдельно от CLI-механизма `startup.rs`/`enable-startup`.
...
- `src/platform/macos/` - вся OS-специфика и unsafe-код в одном месте: `scroll_events.rs` (маппинг полей CGEvent: прочитать событие, записать решение), `permissions.rs` (Accessibility + Input Monitoring TCC), `hid.rs` (IOHIDManager wheel monitor: атрибуция дискретного скролла конкретному vendor/product ID и имени устройства для `device_rules`/Debug Console), `startup.rs` (LaunchAgent автозапуск для headless CLI: `enable-startup`/`disable-startup`, таргетит `run`), `event_tap.rs` (runtime-цикл CGEventTap; config `Arc<RwLock<AppConfig>>`, debug decision recording for GUI builds, `install_and_run` сам берет `daemon_lock` перед HID-монитором и созданием tap), `debug_log.rs` (только `gui`: bounded in-memory ring buffer на 500 scroll decisions для Debug Console, локально и без сети), `daemon_lock.rs` (`flock`-lock: гарантирует, что никогда не запущено два живых `CGEventTap` одновременно; тот же primitive переиспользован для отдельного `ui.lock`), `quit_handler.rs` (перехват AppleEvent quit, чтобы Cmd-Q/Dock quit прятали окно и не обходили tray-only quit), `login_item.rs` (только `gui`: тонкая обертка над `SMAppService.mainAppService()` для регистрации `.app`-бандла как login item), `tray.rs` (только `gui`: AppKit `NSStatusItem`, custom opposing-arrows template glyph, separate colored status-dot, rich native menu with Reverse Scrolling, Devices submenu, Open Settings, Open Debug Console, Quit, and option-click Debug Console shortcut).
...
- `recommendation.md` - 860 пунктов backlog/review (500 базовых + N01-N360 после автозапуска, menu bar, bundle identity, lifecycle, design-handoff review и финального sync) + верифицированные находки итераций ревью.
...
- `debug_log` уже есть как GUI-only local diagnostics ring buffer; следующий шаг - вынести его за интерфейс диагностики, чтобы event tap не знал UI-детали.
...
src/
  ...
  platform/
    ...
    macos/
      ...
      debug_log.rs                 локальный ring buffer Debug Console (gui only)
...
start
  ...
  -> for gui builds: record a local Debug Console decision row in the in-memory ring buffer
...
проблем, ошибок и улучшений плюс follow-up `N01-N360`. Блок ниже зеркалит
...
- repeated review fixed the recommendation counter so exactly 500 backlog items are counted (later extended by N01-N360 after startup, menu bar, bundle identity, lifecycle/design review and final sync, total 860);
