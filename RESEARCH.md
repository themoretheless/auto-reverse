...
src/scroll_dynamics.rs                    pure per-axis discrete-wheel state machine
...
`scroll_scheduler.rs` не принимает решений о кривой. Он получает готовые
samples от pure engine, помечает synthetic event, пишет только
`DeltaAxis1/2`, отбрасывает stale generation и fail-open отключается при любой
ошибке. Это сохраняет SRP и не затягивает CoreGraphics в domain layer.
...
2. [Next R10] Снять callback latency через public `CGGetEventTapList`.
3. [Partial] Transfer-function lab и constant baseline готовы (`R04-R05`);
   ScrollTest-style task harness остается в `R06-R08`.
4. Собрать matrix реальных устройств, частот событий и приложений.
...
1. Реализовать pure per-axis engine с bounded `dt`.
2. Проверить signed-distance conservation, direction reset, opposite-input
   cancellation, gap split и stop threshold property tests.
3. Добавить tagged scheduler только для discrete wheel и выключить его по
   умолчанию.
4. При любой неопределенности пропускать исходный event без smoothing.
...
Пакет 1 (`R01-R05`) реализован: trace/replay/lab не меняют live scroll path.
Следующий пакет - `R06-R10`: ScrollTest task harness, known/unknown target
режимы, distance/tolerance matrix, event-rate distribution и public tap latency.
