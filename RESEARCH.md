...
1. Добавить privacy-bounded trace schema и pure replay.
2. Снять callback latency через public `CGGetEventTapList`.
3. Построить transfer-function и ScrollTest-style benchmark harness.
...
Первый исполнимый пакет: `R01-R12` и `R57-R58`. Он дает trace/replay,
latency measurements, benchmark и migration/property safety, но еще не меняет
ощущение скролла. Только после его результатов стоит брать `R14-R29`.
